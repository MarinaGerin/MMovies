...
document.getElementById('pesquisar').addEventListener('submit', async function (event) {
    ...
    if (resultados.length > 0) {
        ...
        resultados.forEach(resultado => {
            let poster = resultado.poster_path ? `https://image.tmdb.org/t/p/original/${resultado.poster_path}` : 'img/no-poster.png';

            divResultados.innerHTML +=
                `<div class="col">
                    <a href="detalhes.html?id=${resultado.id}&media=${resultado.media_type}" class="text-decoration-none">
                        <div class="card bg-dark h-100">
                            <img src="${poster}" class="card-img-top" style="height:403px">
