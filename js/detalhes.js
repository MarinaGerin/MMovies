...
async function getMovie() {
    ...
    movie.genres.forEach(genre => {
        detalhes.innerHTML += `<button class='btn btn-lg btn-outline-warning me-2'>${genre.name}</button>`
    });
    ...
    for (let i = 0; i < elenco.length; i++) {
        let foto = elenco[i].profile_path ? `http://image.tmdb.org/t/p/original/${elenco[i].profile_path}` : 'img/no-photo-cast.png';
        elencoContainer.innerHTML +=
